 ...
 *                                                                            *
 *   !! WARNING !!                                                            *
 *   This jcon is current density/sqrt(4 pi), similar to B^i in code units.   *
...
void current_calc()
{
  ...
  if (nstep == 0) {
    ZLOOP {
      for (int mu = 0; mu < NDIM; mu++) jcon[i][j][k][mu] = 0.;
    }
    return;
  }
  ...
  ZSLOOP(-1,N1-1+NG,-1,N2-1+NG,-1,N3-1+NG) {
    PLOOP {
      Pa[i][j][k][ip] = 0.5*(P[i][j][k][ip] + Psave[i][j][k][ip]);
    }
  }
...
double Fcon_calc(double *prim, int mu, int nu, int i, int j, int k)
{
  ...
  for (int kap = 0; kap < NDIM; kap++) {
    for (int lam = 0; lam < NDIM; lam++) {
      dFcon = (-1./ggeom[i][j][CENT].g)*antisym(mu,nu,kap,lam)*ucov[kap]*bcov[lam];
      Fcon += dFcon;
    }
  }
