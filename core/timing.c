...
void report_performance()
{
  if (mpi_myrank() == 0) {
    ...
    fprintf(stdout, "   UPDATE:   %8.4g s (%.4g %%)\n", 
      times[TIMER_UPDATE]/dnstep, 100.*times[TIMER_UPDATE]/times[TIMER_ALL]);
    fprintf(stdout, "   FLUXCALC: %8.4g s (%.4g %%)\n", 
      times[TIMER_FLUXCALC]/dnstep, 100.*times[TIMER_FLUXCALC]/times[TIMER_ALL]);
    fprintf(stdout, "   FIXUP:    %8.4g s (%.4g %%)\n", 
      times[TIMER_FIXUP]/dnstep,  100.*times[TIMER_FIXUP]/times[TIMER_ALL]);
    fprintf(stdout, "   BOUND:    %8.4g s (%.4g %%)\n", 
      times[TIMER_BOUND]/dnstep,  100.*times[TIMER_BOUND]/times[TIMER_ALL]);
    fprintf(stdout, "   DIAG:     %8.4g s (%.4g %%)\n", 
      times[TIMER_DIAG]/dnstep,   100.*times[TIMER_DIAG]/times[TIMER_ALL]);
    ...
    fprintf(stdout, "     SECOND: %e\n", 
      N1TOT*N2TOT*N3TOT/(times[TIMER_ALL]/dnstep));
    fprintf(stdout, "*********************************\n\n");
  }
}
