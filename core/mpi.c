...
void init_mpi()
{
  ...
  int radG_X1_subsizes[4] = {NGR,N2+2*NGR,N3+2*NGR,NDIM};
  ...
  int radG_X2_subsizes[4] = {N1+2*NGR,NGR,N3+2*NGR,NDIM};
  ...
  int radG_X3_subsizes[4] = {N1+2*NGR,N2+2*NGR,NGR,NDIM};
  ...
  MPI_Datatype type[PH_ELEM] = {MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                          MPI_DOUBLE, MPI_INT, MPI_INT, MPI_DOUBLE, MPI_INT,
                          mpi_pointer_type};
  int blocklen[PH_ELEM] = {NDIM*NSUP, NDIM*NSUP, NDIM*NSUP, 1, 1, 1, NDIM, 1,
                           1, 1};
  ...
  MPI_Get_address(&(tmp.nscatt),     &(disp[5]));
  MPI_Get_address(tmp.origin,        &(disp[6]));
  MPI_Get_address(&(tmp.t0),         &(disp[7]));
  MPI_Get_address(&(tmp.is_tracked), &(disp[8]));
  MPI_Get_address(&(tmp.next),       &(disp[9]));
  MPI_Aint base;
...
void sync_mpi_boundaries_X1L(grid_prim_type Pr) {
  ...
  MPI_Sendrecv(&Pr[N1][NG][NG][0], 1, prim_face_subtype[1], neighbors[2][1][1],
    0, &Pr[0][NG][NG][0], 1, prim_face_subtype[1], neighbors[0][1][1], 0, comm,
    &status);
  MPI_Sendrecv(&pflag[N1][NG][NG], 1, pflag_face_subtype[1], neighbors[2][1][1],
    0, &pflag[0][NG][NG], 1, pflag_face_subtype[1], neighbors[0][1][1], 0, comm,
    &status);
}
...
void sync_mpi_boundaries_X1R(grid_prim_type Pr) {
  ...
  MPI_Sendrecv(&Pr[NG][NG][NG][0], 1, prim_face_subtype[1], neighbors[0][1][1],
    1, &Pr[N1+NG][NG][NG][0], 1, prim_face_subtype[1], neighbors[2][1][1], 1,
    comm, &status);
  MPI_Sendrecv(&pflag[NG][NG][NG], 1, pflag_face_subtype[1], neighbors[0][1][1],
    1, &pflag[N1+NG][NG][NG], 1, pflag_face_subtype[1], neighbors[2][1][1], 1,
    comm, &status);
}
...
void sync_mpi_boundaries_X2L(grid_prim_type Pr) {
  ...
  MPI_Sendrecv(&pflag[0][N2][NG], 1, pflag_face_subtype[2], neighbors[1][2][1],
    2, &pflag[0][0][NG], 1, pflag_face_subtype[2], neighbors[1][0][1], 2, comm,
    &status);
}
...
void sync_mpi_boundaries_X2R(grid_prim_type Pr) {
  ...
  MPI_Sendrecv(&pflag[0][NG][NG], 1, pflag_face_subtype[2], neighbors[1][0][1],
    3, &pflag[0][N2+NG][NG], 1, pflag_face_subtype[2], neighbors[1][2][1], 3,
    comm, &status);
}
...
void sync_mpi_boundaries_X3R(grid_prim_type Pr) {
  ...
  MPI_Sendrecv(&Pr[0][0][NG][0], 1, prim_face_subtype[3], neighbors[1][1][0], 5,
    &Pr[0][0][N3+NG][0], 1, prim_face_subtype[3], neighbors[1][1][2], 5, comm,
    &status);
...
void sync_mpi_photons(struct of_photon **ph_mpi, double t, double dt)
{
  ...
  for (int dir = 1; dir < NDIM; dir++) {
    ...
    MPI_Sendrecv(&nph_send_R[dir], 1, MPI_INT, neighbor_R, 0,
      &nph_recv_L[dir], 1, MPI_INT, neighbor_L, 0, comm,
      &status);
    ...
    MPI_Sendrecv(ph_send_buf, nph_send_R[dir], mpi_photon_type, neighbor_R,
      0, ph_recv_buf, nph_recv_L[dir], mpi_photon_type, neighbor_L, 0, comm, 
      &status);
    free(ph_send_buf);
    ...
    MPI_Sendrecv(&nph_send_L[dir], 1, MPI_INT, neighbor_L, 0,
      &nph_recv_R[dir], 1, MPI_INT, neighbor_R, 0, comm,
      &status);
    ...
    MPI_Sendrecv(ph_send_buf, nph_send_L[dir], mpi_photon_type, neighbor_L,
      0, ph_recv_buf, nph_recv_R[dir], mpi_photon_type, neighbor_R, 0, comm, 
      &status);
    free(ph_send_buf);
    ...
    while (ph_tmp_recv != NULL) {
      get_X_K_interp(ph_tmp_recv, t+dt, X, Kcov, Kcon);
...
int mpi_is_periodic(int dir) {
  return periodic[dir-1];
}

#if RADIATION
static double nuLnu_local[MAXNSCATT+1][NTH][NPHI][NU_BINS_SPEC];
void mpi_reduce_nuLnu() {
  MPI_Reduce(nuLnu, nuLnu_local, (MAXNSCATT+1)*NTH*NPHI*NU_BINS_SPEC, MPI_DOUBLE,
    MPI_SUM, 0, comm);
  memcpy(nuLnu, nuLnu_local, (MAXNSCATT+1)*NTH*NPHI*NU_BINS_SPEC*sizeof(double));
}
#endif // RADIATION
