...
void prim_meta(FILE* fp, const char vnams[NVAR][STRLEN], int indx);
void scalar_meta(FILE* fp, const char* name, const char* sourcename, int precision);
void vec_meta(FILE* fp, const char* name);
void tensor_meta(FILE* fp, const char* name, const char* sourcename, int precision);

void vec_component(FILE* fp, const char* name, int indx);
void tensor_component(FILE* fp, const char* name,
  const char* sourcename, int precision, int mu, int nu);

void write_xml_file(int dump_id, double t, const char vnams[NVAR][STRLEN])
{
  // I know I don't need to do this every time... but it doesn't cost anything ~JMM
  ...
  sprintf(dname, "../dumps/dump_%08d.h5", dump_id);
  sprintf(gname, "../dumps/grid.h5");
...
void geom_meta(FILE* fp)
{
  ...
  for (int d = 1; d < NDIM; d++) coord_meta(fp, d);
  fprintf(fp, "      </Geometry>\n");
}
...
void coord_meta(FILE* fp, int indx)
{
  fprintf(fp, "        <DataItem ItemType=\"Hyperslab\" Dimensions=\"%d %d %d\" Type=\"Hyperslab\">\n",
    iX1_max+1, N2TOT+1, N3TOT+1);
...
void vec_component(FILE* fp, const char* name, int indx)
{
  fprintf(fp, "      <Attribute Name=\"%s_%d\" AttributeType=\"Scalar\" Center=\"Cell\">\n",
    name, indx);
  fprintf(fp, "        <DataItem ItemType=\"Hyperslab\" Dimensions=\"%d %d %d\" Type=\"Hyperslab\">\n",
    iX1_max, N2TOT, N3TOT);
  fprintf(fp, "          <DataItem Dimensions=\"3 4\" Format=\"XML\">\n");
...
void tensor_component(FILE* fp, const char* name, const char* sourcename, int precision, int mu, int nu)
{
  fprintf(fp, "      <Attribute Name=\"%s_%d%d\" AttributeType=\"Scalar\" Center=\"Cell\">\n",
    name, mu, nu);
  fprintf(fp, "        <DataItem ItemType=\"Hyperslab\" Dimensions=\"%d %d %d\" Type=\"Hyperslab\">\n",
    iX1_max, N2TOT, N3TOT);
